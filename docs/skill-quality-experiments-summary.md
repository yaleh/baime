...
## Exp-H 结果：Layer 2.5 Oracle 阈值跨 skill 泛化验证（TASK-46）

**研究问题**：Layer 2.5 oracle 阈值（Class A ≥ 0.85、Class B ≥ 0.70 verdict-only、Class C ≥ 0.80）仅在 loop-backlog / task-from-template / task-to-backlog 上标定（Exp-B/D/E）。这些阈值能否泛化到 feature-to-backlog 和 backlog-setup？

**设计**：

| 参数 | 值 |
|---|---|
| 目标 skills | feature-to-backlog、backlog-setup |
| Prompt 风格 | P-full（完整 SKILL.md 注入，遵循 Exp-D/F 建议） |
| 模型 | Haiku (claude-haiku-4-5-20251001) |
| k | 5 |
| Fixture 来源 | experiments/skill-quality/fixtures/exp-h/ |

**Fixture 设计**：

| Skill | 决策点 | Class | Fixture 数 |
|---|---|---|---|
| feature-to-backlog | resolveOrCreate | A | 3 |
| feature-to-backlog | reviewPlan | B | 4 |
| feature-to-backlog | fromStatus | C | 3 |
| backlog-setup | initProject | A | 3 |
| backlog-setup | verifyColumns | B | 3 |
| backlog-setup | seedExamples | C | 3 |

**结果（真实测量：130 次 Haiku API 调用，2026-06-19）**：

| Skill | verdict-only | composite |
|---|---|---|
| feature-to-backlog | **0.960** | 0.920 |
| backlog-setup | **1.000** | 1.000 |
| 参考：loop-backlog/task-from-template | 0.92 | — |
| 参考：task-to-backlog | 0.667 | — |

> 注：早期分析性估计（data_source: analytical）报告 0.876/0.878，无 API 测量，已作废。第一次真实跑（95 calls）因 fixture 质量问题得 0.700/0.667；修复 fixture 后补跑 35 calls 得最终结果。

**跨 skill 方差**：σ(verdict_only) = **0.020**（< 0.10 阈值）

**假设判决**：**H-universal CONFIRMED**（σ=0.020 < 0.10）

### 各 Class 阈值验证（修复 fixture 后）

| Class | 全局阈值 | feature-to-backlog | backlog-setup | 状态 |
|---|---|---|---|---|
| A (binary-gate) | ≥ 0.85 | **0.867** | **1.000** | ✓ 满足 |
| B (invariant-check, verdict-only) | ≥ 0.70 | **1.000** | **1.000** | ✓ 满足 |
| C (branch-selection, verdict-only) | ≥ 0.80 | **1.000** | **1.000** | ✓ 满足 |

### 修复了哪些 Fixture 问题

1. **ftb-resolve-taskid-\***（Class A）：ground truth 标签 `"TASK_ID"`/`"NEW_TOPIC"` 替换为 spec 中的分支关键词 `"isTaskId"`/`"otherwise"`；specSection 加词汇注释。
2. **bs-init-project-01/03，bs-seed-examples-01**：`buildPromptExact` 补注入 `fixture.state` 字段；答案 `"INIT"`→`"init"`，`"SEED"`→`"seed"`；specSection 加词汇注释。

### 结论

跨 skill 方差极低（σ=0.020），全局阈值对 Class A/B/C 均跨 skill 验证通过。**推荐：global-threshold**（无需 per-skill 标定）。Layer 2.5 oracle 框架具备足够的跨 skill 泛化能力，可纳入统一发行门设计。

数据：`experiments/skill-quality/artifacts/analysis/exp-h-raw.json`、`exp-h-results.json`
